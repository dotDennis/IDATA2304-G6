 ...
 * 
 * <p>Parts of code generated by AI.
 * And modified to fit our understanding, extra research was necessary.
 * 
 * <p>The function of this class is to let the GUI layer create embedded sensor
 * nodes
...
public class EmbeddedNodeService {
    ...
    for (ControlNodeConfig.SensorNodeEntry nodeEntry : config.getSensorNodes()) {
      try {
        ...
        if (errorHandler != null) {
          errorHandler.accept("Failed to restore node " 
              + nodeEntry.getId() + ": " + e.getMessage());
        }
      }
    }
  }
   ...
   */
  public ControlNodeConfig.Entry buildConfigEntry(String id, String displayName,
    ...
    for (EmbeddedSensorNodeManager.EmbeddedNode node : manager.listNodes()) {
      ...
      for (Sensor sensor : node.getSensorNode().getSensors()) {
        nodeEntry.getSensors().add(
            new ControlNodeConfig.SensorEntry(sensor.getDeviceId(), sensor.getDeviceType().name(),
                sensor.getUpdateInterval()));
      }
      for (Actuator actuator : node.getSensorNode().getActuators()) {
        nodeEntry.getActuators().add(
            new ControlNodeConfig.DeviceEntry(actuator.getDeviceId(),
             actuator.getDeviceType().name()));
      }
